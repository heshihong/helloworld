...
public class LogAspect {
    ...
    //	public void remove()将当前线程局部变量的值删除，目的是为了减少内存的占用，该方法是JDK 5
    //	.0新增的方法。需要指出的是，当线程结束后，对应该线程的局部变量将自动被垃圾回收，所以显式调用该方法清除线程的局部变量并不是必须的操作，但它可以加快内存回收的速度。
    //	protected Object initialValue()返回该线程局部变量的初始值，该方法是一个protected的方法，显然是为了让子类覆盖而设计的。这个方法是一个延迟调用方法，在线程第1次调用get()或set
    //	(Object)时才执行，并且仅执行1次。ThreadLocal中的缺省实现直接返回一个null。
    //	值得一提的是，在JDK5.0中，ThreadLocal已经支持泛型，该类的类名已经变为ThreadLocal<T>。API方法也相应进行了调整，新版本的API方法分别是void set(T value)、T get()
    //	以及T initialValue()。
    ThreadLocal<RequestLogPO> logPOThreadLocal = new ThreadLocal<>();
    ...
    public void doAfterReturning(JoinPoint joinPoint, Object returnData) {
        ...
        int res = requestLogMapper.insertRequestLog(requestLogPO);
        if (res <= 0) {
    ...
    public void doAfterThrowingAdvice(JoinPoint joinPoint, Throwable exception) {
        ...
        requestLogPO.setRespTime(System.currentTimeMillis() - startTime.get());
        // 设置异常信息
        requestLogPO.setExceptionMessage(exception.toString());
        int res = requestLogMapper.insertRequestLog(requestLogPO);
        if (res <= 0) {
            logger.error("插入接口日志失败！");
        }
        //目标方法名：
        logger.info("接口调用异常：" + exception.toString());
    }
}
